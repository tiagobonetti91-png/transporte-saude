...
export function ModalViagem({ item, veiculos, motoristas, pacientes, destinos, onSave, onClose }) {
  const [form,setForm]=useState(item
    ?{data:item.data,horarioSaida:item.horarioSaida,veiculoId:item.veiculo.id,motoristaId:item.motorista.id,passageiros:item.passageiros,status:item.status}
  ...
  function addPax() {
    ...
    setForm(f=>({...f,passageiros:[...f.passageiros,{id:Date.now(),paciente:selPac,destino:selDest,horarioChegada:horCheg,status:"indefinido",assinatura:null}]}));
    ...
    <div style={S.modal}><div style={{ ...S.modalBox, maxWidth:620 }}>
      ...
      <div style={{ background:"#0f2040", borderRadius:14, padding:14, border:"1px solid #1e3a5f", marginBottom:14 }}>
        ...
        <div style={{ display:"flex", gap:10, alignItems:"flex-end" }}>
          ...
          <Btn onClick={addPax} disabled={!selPac||!selDest||form.passageiros.length>=cap} color="#3b82f6">+ Add</Btn>
        </div>
      </div>
