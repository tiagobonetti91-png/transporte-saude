...
export function ModalDestino({ item, onSave, onClose }) {
  ...
  return (
    <div style={S.modal}><div style={{ ...S.modalBox, maxWidth:600 }}>
      <ModalHdr title={item?"Editar Clinica/Hospital":"Nova Clinica/Hospital"} onClose={onClose}/>
      ...
      <Inp label="Endereco" value={f.endereco} onChange={e=>setF(p=>({...p,endereco:e.target.value}))}/>
...
export function ModalVeiculo({ item, onSave, onClose }) {
  ...
  return (
    <div style={S.modal}><div style={S.modalBox}>
      <ModalHdr title={item?"Editar Veiculo":"Novo Veiculo"} onClose={onClose}/>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        <Inp label="Placa" value={f.placa} onChange={e=>setF(p=>({...p,placa:e.target.value}))} placeholder="ABC-1234"/>
        <Sel label="Tipo" value={f.tipo} onChange={v=>setF(p=>({...p,tipo:v}))} options={["Carro","Van Pequena","Van Grande","Micro-onibus"].map(t=>({value:t,label:t}))}/>
      </div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:12 }}>
        ...
        <Sel label="Combustivel" value={f.combustivel} onChange={v=>setF(p=>({...p,combustivel:v}))} options={COMBUSTIVEIS.map(c=>({value:c,label:c}))}/>
...
export function ModalViagem({ item, veiculos, motoristas, pacientes, destinos, onSave, onClose, salvando }) {
  ...
  function addPax() {
    ...
    if(totalVagas+vagasNecessarias>cap){ alert("Veiculo lotado! Capacidade: "+cap+" vagas."); return; }
  ...
  return (
    <div style={S.modal}><div style={{ ...S.modalBox, maxWidth:620 }}>
      ...
      {item && <div style={{ background:VIAGEM_STATUS[item.status]?.color+"18", border:"1px solid "+VIAGEM_STATUS[item.status]?.color+"44", borderRadius:10, padding:"8px 14px", marginBottom:16, fontSize:12, color:VIAGEM_STATUS[item.status]?.color }}>Status atual: <b>{VIAGEM_STATUS[item.status]?.label}</b></div>}
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        <Inp label="Data" type="date" value={form.data} onChange={e=>setForm(f=>({...f,data:e.target.value}))}/>
        <Inp label="Saida" type="time" value={form.horarioSaida} onChange={e=>setForm(f=>({...f,horarioSaida:e.target.value}))}/>
      </div>
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
        ...
        <Sel label="Veiculo" value={form.veiculoId} onChange={v=>setForm(f=>({...f,veiculoId:parseInt(v)}))} options={veiculos.map(v=>({value:v.id,label:v.placa+" ("+v.capacidade+"lug.)"}))}/>
      </div>
      ...
      <div style={{ background:"#0f2040", borderRadius:14, padding:14, border:"1px solid #1e3a5f", marginBottom:14 }}>
        <SecTitle>Adicionar Passageiro ({totalVagas}/{cap} vagas)</SecTitle>
        ...
        <AutoC label="Destino" placeholder="Hospital, clinica ou especialidade..." value={selDest?selDest.nome:destQ} results={destRes}
        ...
        <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
          <div>
            <label style={S.label}>Horario Chegada</label>
        ...
        <div style={{ marginTop:10 }}>
          ...
          <input value={localEmbarque} onChange={e=>setLocalEmbarque(e.target.value)} placeholder="Ex: Rua das Flores, 123" style={S.input}/>
        </div>
        <div style={{ marginTop:10 }}>
          ...
          {acompList.map(a=>(
            <div key={a.id} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",background:"#0a1628",borderRadius:8,padding:"6px 12px",marginTop:6,fontSize:12,color:"#e2e8f0" }}>
              <span>Acomp: {a.nome}</span>
              <button onClick={()=>removeAcomp(a.id)} style={{ background:"none",border:"none",color:"#f87171",cursor:"pointer",fontSize:16 }}>x</button>
            </div>
          ))}
        </div>
        <div style={{ marginTop:10 }}>
      ...
      {form.passageiros.length>0 && (
        <div>
          ...
          {form.passageiros.map((p,i)=>(
            <div key={p.id} style={{ background:"#0f2040", borderRadius:10, padding:"10px 14px", marginBottom:8 }}>
              <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
                <div>
                  <div style={{ fontSize:13, fontWeight:600, color:"#e2e8f0" }}>{p.paciente.nome}</div>
                  <div style={{ fontSize:11, color:"#a78bfa" }}>{p.destino.nome} -- {p.horarioChegada}</div>
                  <div style={{ fontSize:11, color:"#64748b" }}>
                    {p.localEmbarque ? p.localEmbarque+" -- " : ""}
                  ...
                  {p.acompanhantes?.length>0 && (
                    <div style={{ marginTop:4 }}>
                      {p.acompanhantes.map(a=>(
                        <div key={a.id} style={{ fontSize:11, color:"#94a3b8" }}>Acomp: {a.nome}</div>
                      ))}
                    </div>
                  )}
                </div>
                <button onClick={()=>setForm(f=>({...f,passageiros:f.passageiros.filter((_,j)=>j!==i)}))} style={{ background:"none",border:"none",color:"#f87171",cursor:"pointer",fontSize:18,marginLeft:8 }}>x</button>
              </div>
            </div>
          ))}
        </div>
      ...
      <div style={{ display:"flex", gap:10, marginTop:12 }}>
        <Btn full onClick={save} color="#10b981" disabled={form.passageiros.length===0||salvando}>{salvando?"Salvando...":"Salvar Viagem"}</Btn>
        <Btn full onClick={onClose} color="#475569">Cancelar</Btn>
      </div>
    </div></div>
  );
  ...
  return (
    <div style={S.modal}><div style={S.modalBox}>
      ...
      <div style={{ background:"#0f2040", borderRadius:12, padding:"12px 14px", marginBottom:16 }}>
        <div style={{ fontSize:12, color:"#64748b" }}>Veiculo: <span style={{color:"#38bdf8",fontWeight:600}}>{veiculo?.placa} -- {veiculo?.modelo}</span></div>
...
export function ModalAssinatura({ passageiro, nomeOverride, onSave, onClose }) {
  ...
  function getPos(e,rect) { const src=e.touches?e.touches[0]:e; return {x:src.clientX-rect.left,y:src.clientY-rect.top}; }
  function startDraw(e) { e.preventDefault(); const rect=e.currentTarget.getBoundingClientRect(); const pos=getPos(e,rect); setDrawing(true); setPoints(prev=>[...prev,[pos]]); }
  function draw(e) { e.preventDefault(); if(!drawing) return; const rect=e.currentTarget.getBoundingClientRect(); const pos=getPos(e,rect); setPoints(prev=>{const n=[...prev];n[n.length-1]=[...n[n.length-1],pos];return n;}); setSigned(true); }
  ...
  return (
    <div style={S.modal}><div style={S.modalBox}>
      ...
      <div style={{ marginBottom:14 }}>
        {nomeOverride
          ? <div><div style={{ fontSize:11,color:"#a78bfa",marginBottom:2 }}>ACOMPANHANTE DE {passageiro.paciente.nome}</div><div style={{ fontSize:14,fontWeight:600,color:"#e2e8f0" }}>{nomeOverride}</div></div>
      ...
      <div style={{ display:"flex", gap:10 }}>
        ...
        <Btn onClick={clear} color="#475569" small>Limpar</Btn>
        <Btn onClick={onClose} color="#475569" small>Cancelar</Btn>
      </div>
      <div style={{ marginTop:12,fontSize:11,color:"#475569",textAlign:"center" }}>A assinatura confirma o embarque e serve como comprovante.</div>
    </div></div>
  );
}
